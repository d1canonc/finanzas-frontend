...
const CATS = [
  {id:"food",label:"🍽️ Comida",color:"#FF6B6B"},{id:"transport",label:"🚗 Transporte",color:"#4ECDC4"},
  {id:"entertainment",label:"🎬 Entrete.",color:"#FFE66D"},{id:"health",label:"💊 Salud",color:"#95E1D3"},
  {id:"shopping",label:"🛍️ Compras",color:"#F38181"},{id:"services",label:"📱 Servicios",color:"#A8D8EA"},
  {id:"education",label:"📚 Educación",color:"#AA96DA"},{id:"other",label:"📦 Otro",color:"#C7F2A4"},
];
const ACCS = [{id:"credit",label:"💳 Crédito",color:"#6b6bff"},{id:"savings",label:"🏦 Ahorros",color:"#4ade80"},{id:"both",label:"⚡ Ambas",color:"#facc15"}];
...
function useAPI() {
  ...
  return useCallback(async (method,path,body) => {
    ...
    if(res.status===401){localStorage.removeItem("fin_token");window.location.href="/";}
...
const CSS=`
  ...
  *{box-sizing:border-box;margin:0;padding:0;}body{background:#090910;}
  ...
  .tab.active{background:#e8e4dc;color:#090910;}.tab:hover:not(.active){background:#1e1e2a;color:#e8e4dc;}
  .mtab{cursor:pointer;padding:5px 11px;border-radius:16px;font-size:11px;border:none;background:transparent;color:#555;white-space:nowrap;transition:all .18s;font-family:'DM Sans',sans-serif;}
  .mtab.active{background:#e8e4dc;color:#090910;font-weight:600;}.mtab:hover:not(.active){color:#e8e4dc;background:#1e1e2a;}
  ...
  .inp:focus{border-color:#6b6bff;}textarea.inp{resize:vertical;min-height:80px;font-size:13px;line-height:1.6;}select.inp{appearance:none;}
  ...
  .bsm{padding:6px 12px;font-size:12px;}.bdel{background:transparent;border:none;color:#2a2a3a;cursor:pointer;padding:3px 7px;font-size:13px;transition:color .15s;}.bdel:hover{color:#f87171;}
  .row{display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #181824;animation:fi .25s ease;}.row:last-child{border-bottom:none;}
  @keyframes fi{from{opacity:0;transform:translateY(-4px)}to{opacity:1;transform:translateY(0)}}
  .toast{position:fixed;bottom:22px;left:50%;transform:translateX(-50%);background:#e8e4dc;color:#090910;padding:10px 20px;border-radius:10px;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:600;z-index:9999;animation:su .25s ease;box-shadow:0 4px 24px rgba(0,0,0,.6);white-space:nowrap;}
  ...
  .segtab{flex:1;cursor:pointer;border:none;border-radius:7px;padding:7px 4px;font-family:'DM Sans',sans-serif;font-size:12px;font-weight:500;background:transparent;color:#555;transition:all .15s;}
  .segtab.active{background:#1e1e2c;color:#e8e4dc;}
  .chip{display:inline-flex;align-items:center;padding:2px 7px;border-radius:20px;font-size:10px;font-family:'DM Sans',sans-serif;font-weight:600;}
  .pulse{animation:pu 1.4s ease-in-out infinite;}@keyframes pu{0%,100%{opacity:1}50%{opacity:.3}}
  .stat{font-family:'Playfair Display',serif;font-weight:700;}.label{font-size:10px;color:#444;text-transform:uppercase;letter-spacing:.8px;}
  ...
  .ag{background:#0a1e0e;border:1px solid #1a4a22;border-radius:12px;padding:12px 14px;font-size:13px;line-height:1.5;margin-bottom:10px;}
  .qbtn{cursor:pointer;background:#181824;border:1px solid #1e1e2c;border-radius:10px;padding:10px 12px;font-family:'DM Sans',sans-serif;font-size:12px;color:#aaa;text-align:left;transition:all .15s;width:100%;}.qbtn:hover{border-color:#6b6bff;color:#e8e4dc;}
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    if(p.get("error")) showT("Error de autenticación","err");
  },[]);
  ...
  const load = useCallback(async()=>{
    ...
    try {
      const [me,t,i,pen,comp] = await Promise.all([
        api("GET","/api/me"), api("GET","/api/transactions"), api("GET","/api/installments"),
        api("GET","/api/pending"), api("GET","/api/comparison"),
      ]);
      setUser(me); setSettings(me.settings); setSettingsForm({...me.settings,budgets:me.settings.budgets||{}}); setTxs(t); setInsts(i); setPending(pen); setComparison(comp);
      // Load recurring in background
      api("GET","/api/recurring").then(setRecurring).catch(()=>{});
    } catch(e){if(!e.message?.includes("401"))showT("Error cargando","err");}
    setLoading(false);
  },[token,api]);
  ...
  const instChargeForMonth = mo => insts.reduce((sum,inst)=>{
    const el=mDiff(inst.start_month,mo);
    if(el>=0&&el<inst.months){ const r=inst.interest_rate/100; return sum+(r>0?inst.total_amount*r*Math.pow(1+r,inst.months)/(Math.pow(1+r,inst.months)-1):inst.total_amount/inst.months); }
    return sum;
  },0);
  ...
  const cutDay = S.cut_day||25; const payDay = S.pay_day||10;
  ...
  const addTx = async()=>{
    ...
    await api("POST","/api/transactions",{...txForm,amount}); setTxForm({name:"",amount:"",category:"food",account:"credit",type:"expense",date:today()}); await load(); showT("Registrado ✓");
  };
  const addInst = async()=>{
    ...
    await api("POST","/api/installments",{...instForm,total_amount:total,months:parseInt(instForm.months),interest_rate:parseFloat(instForm.interest_rate)||0}); setInstForm({name:"",total_amount:"",months:3,interest_rate:0,category:"shopping",account:"credit",start_month:getCM()}); await load(); showT("Cuotas registradas ✓");
  };
  ...
  const saveSettings = async()=>{ await api("PUT","/api/settings",{...settingsForm,budgets:{...(S.budgets||{}),...Object.fromEntries(Object.entries(budgetIn).map(([k,v])=>[k,parseInt(v?.replace(/\D/g,""))||0]).filter(([,v])=>v>0))}}); await load(); showT("Guardado ✓"); };
  const answerPending=async(p)=>{ const a=pendingAns[p.id]||{}; if(!a.months){showT("Indica cuotas","err");return;} await api("POST",`/api/pending/${p.id}/answer`,a); await load(); showT("Registrado ✓"); };
  const doSync=async()=>{ setSyncing(true); await api("POST","/api/sync"); setTimeout(async()=>{await load();setSyncing(false);showT("Sync completo ✓");},9000); };
  const askAdvisor=async(q2)=>{ setAdviceLoad(true);setAdvice(""); try{const r=await api("POST","/api/advisor",{question:q2||adviceQ});setAdvice(r.advice);}catch{setAdvice("Error conectando.");} setAdviceLoad(false); };
  const simulate=async()=>{
    ...
    try{const r=await api("POST","/api/simulate",{...simForm,amount,months:parseInt(simForm.months),interestRate:parseFloat(simForm.interestRate)||0});setSimResult(r);}catch{showT("Error en simulador","err");}
    setSimLoad(false);
  };

  // ── Login screen ──────────────────────────────────────────────────────────
  if(!token) return (
    <div style={{fontFamily:"'DM Sans',sans-serif",background:"#090910",minHeight:"100vh",color:"#e8e4dc",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <style>{CSS}</style>
      <div style={{maxWidth:340,width:"100%",textAlign:"center"}}>
        <div style={{fontFamily:"'Playfair Display',serif",fontSize:38,fontWeight:900,marginBottom:6}}>finanzas<span style={{color:"#6b6bff"}}>.</span></div>
        <div style={{fontSize:13,color:"#444",marginBottom:8,lineHeight:1.6}}>Gestión inteligente de tu tarjeta y ahorros</div>
        <div style={{display:"flex",flexDirection:"column",gap:8,fontSize:12,color:"#555",marginBottom:28,textAlign:"left",background:"#11111a",border:"1px solid #1e1e2c",borderRadius:12,padding:14}}>
          {["✅ Correos bancarios leídos automáticamente","✅ Cuotas con y sin interés","✅ Notificaciones push en tu celular","✅ Asesor IA con tus datos reales","✅ 100% gratis — sin tarjeta de crédito"].map(t=><div key={t} style={{fontSize:12}}>{t}</div>)}
        </div>
        ...
        <div style={{fontSize:11,color:"#2a2a3a",lineHeight:1.6}}>🔒 OAuth seguro — nunca guardamos tu contraseña</div>
      </div>
    </div>
  );

  if(loading) return <div style={{background:"#090910",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center"}}><style>{CSS}</style><div className="pulse" style={{fontFamily:"'Playfair Display',serif",fontSize:24,color:"#6b6bff"}}>finanzas.</div></div>;

  return (
    <div style={{fontFamily:"'DM Sans',sans-serif",background:"#090910",minHeight:"100vh",color:"#e8e4dc"}}>
      <style>{CSS}</style>
      ...
      <div style={{maxWidth:480,margin:"0 auto",padding:"13px 13px 90px"}}>
        ...
        {view==="pending"&&(<>
          ...
          {pending.map(p=>{
            const d=JSON.parse(p.parsed_data);
            return <div key={p.id} className="card" style={{marginBottom:10,background:"#160d22",borderColor:"#3d2a5e"}}>
        ...
        {view==="dashboard"&&(<>
          <div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:6}}>
            ...
            {prevMonths.filter(m=>m!==getCM()).map(m=><button key={m} className={`mtab ${month===m?"active":""}`} onClick={()=>setMonth(m)}>{getML(m)}</button>)}
          </div>

          {/* Alerts */}
          {pending.length>0&&<div className="ay" style={{cursor:"pointer"}} onClick={()=>setView("pending")}>❓ <strong>{pending.length} compra(s)</strong> esperan información de cuotas</div>}
          ...
          {S.monthly_income>0&&cExpTotal>S.monthly_income*0.3&&<div className="ay">💡 Gasto en crédito supera el 30% de tu nómina</div>}
          {daysToCut<=3&&<div className="ay">✂️ Fecha de corte en <strong>{daysToCut} día(s)</strong> — saldo actual: {fmt(cExpTotal)}</div>}
          ...
          <div className="card" style={{marginBottom:10,background:"linear-gradient(135deg,#11111a,#18182e)",position:"relative",overflow:"hidden"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
              <div>
                <div className="label">💳 Tarjeta Crédito</div>
            ...
            <div className="bar" style={{height:6,marginBottom:4}}><div style={{height:"100%",width:`${cPct}%`,background:`linear-gradient(90deg,#6b6bff,${cColor})`,borderRadius:99,transition:"width 1.1s cubic-bezier(.4,0,.2,1)"}}/></div>
          ...
          <div className="card" style={{marginBottom:10,background:"linear-gradient(135deg,#0c1a0c,#11221a)"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:S.savings_goal>0?10:0}}>
              <div><div className="label">🏦 Cuenta Ahorros</div><div className="stat" style={{fontSize:26,color:"#4ade80",marginTop:2}}>{fmt(savBal)}</div></div>
              <div style={{textAlign:"right"}}><div style={{fontSize:11,color:"#2a5a2a",marginBottom:4}}>Este mes</div><div style={{fontSize:13,color:"#4ade80",fontWeight:600}}>+{fmt(mSavInc)}</div><div style={{fontSize:13,color:"#f87171",fontWeight:600}}>-{fmt(mSavExp)}</div></div>
            </div>
            {S.savings_goal>0&&<><div style={{fontSize:12,color:"#2a5a2a",marginBottom:5}}>{S.savings_goal_name}: {savGoalPct.toFixed(0)}% · faltan {fmt(S.savings_goal-savBal)}</div><div className="bar" style={{height:5}}><div style={{height:"100%",width:`${savGoalPct}%`,background:"linear-gradient(90deg,#16a34a,#4ade80)",borderRadius:99,transition:"width 1.1s"}}/></div></>}
          </div>

          {/* Balance */}
          {S.monthly_income>0&&<div className="card" style={{marginBottom:10}}>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:14,fontWeight:700,marginBottom:10}}>Balance del mes</div>
            {[["💰 Nómina",S.monthly_income,"#4ade80",100],["💳 Crédito",cExpTotal,"#6b6bff",(cExpTotal/S.monthly_income)*100],["🏦 Débito ahorros",mSavExp,"#4ECDC4",(mSavExp/S.monthly_income)*100]].map(([l,v,c,p])=>(
              <div key={l} style={{marginBottom:8}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:12,color:"#aaa"}}>{l}</span><span style={{fontSize:13,fontWeight:600}}>{fmt(v)}</span></div><div className="bar" style={{height:4}}><div style={{height:"100%",width:`${Math.min(p||0,100)}%`,background:c,borderRadius:99,transition:"width 1s"}}/></div></div>
            ))}
            <div style={{borderTop:"1px solid #1e1e2c",paddingTop:8,display:"flex",justifyContent:"space-between",fontSize:13}}><span style={{color:"#555"}}>Estimado libre</span><span style={{fontWeight:700,color:freeIncome>=0?"#4ade80":"#f87171"}}>{fmt(freeIncome)}</span></div>
          </div>}
          ...
          <div className="card" style={{marginBottom:10}}>
            ...
            {CATS.filter(c=>byCat[c.id]>0).sort((a,b)=>byCat[b.id]-byCat[a.id]).map(cat=>{
              ...
              return <div key={cat.id} style={{marginBottom:9}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:12}}>{cat.label}</span><div><span style={{fontSize:12,fontWeight:600}}>{fmt(amt)}</span>{budget&&<span style={{fontSize:10,color:amt>budget?"#f87171":"#4ade80",marginLeft:5}}>/{fmt(budget)}</span>}</div></div>
          ...
          {/* Month comparison */}
          {comparison.length>1&&<div className="card" style={{marginBottom:10}}>
            ...
            {comparison.map((mc,i)=>{
              ...
              const diff=prev?mc.total-prev.total:0;
              return <div key={mc.month} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:i<comparison.length-1?"1px solid #181824":"none",fontSize:13}}>
                <span style={{color:i===0?"#e8e4dc":"#666"}}>{getML(mc.month)}{i===0?" (actual)":""}</span>
                <div style={{textAlign:"right"}}>
                  <span style={{fontWeight:600}}>{fmt(mc.total)}</span>
                  {prev&&<span style={{fontSize:11,color:diff>0?"#f87171":"#4ade80",marginLeft:6}}>{diff>0?"▲":"▼"}{fmt(Math.abs(diff))}</span>}
                </div>
              </div>;
            })}
          </div>}
          ...
          {/* Recurring */}
          {recurring.length>0&&<div className="card" style={{marginBottom:10}}>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:14,fontWeight:700,marginBottom:10}}>🔄 Suscripciones detectadas</div>
            {recurring.map(r=><div key={r.name} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:"1px solid #181824",fontSize:13}}>
          ...
          <div className="card" style={{marginBottom:10}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
              {projs.map((p,i)=><div key={p.mo} style={{background:"#181824",borderRadius:10,padding:"10px 8px",textAlign:"center",border:`1px solid ${i===0?"#6b6bff33":"#1e1e2c"}`}}>
        ...
        {view==="movimientos"&&(<>
          ...
          <div className="card" style={{marginBottom:12}}>
            <div className="seg" style={{marginBottom:12}}>
              {[["single","💳 Único"],["installment","📋 Cuotas"]].map(([v,l])=><button key={v} className={`segtab ${txTab===v?"active":""}`} onClick={()=>setTxTab(v)}>{l}</button>)}
            </div>
            {txTab==="single"&&<><div className="seg" style={{marginBottom:10}}>{[["expense","📉 Gasto"],["income","📈 Ingreso"]].map(([v,l])=><button key={v} className={`segtab ${txForm.type===v?"active":""}`} onClick={()=>setTxForm(f=>({...f,type:v}))}>{l}</button>)}</div>
            ...
            {txTab==="installment"&&<div style={{display:"flex",flexDirection:"column",gap:8}}>
              ...
              {instForm.total_amount&&(()=>{ const t=parseInt(String(instForm.total_amount).replace(/\D/g,""))||0; const n=parseInt(instForm.months)||1; const r=parseFloat(instForm.interest_rate)||0; const m=r>0?t*(r/100)*Math.pow(1+r/100,n)/(Math.pow(1+r/100,n)-1):t/n; return <div style={{background:"#181824",border:"1px solid #1e1e2c",borderRadius:10,padding:12,fontSize:12,display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}><div><span style={{color:"#555"}}>Cuota: </span><strong>{fmt(m)}/mes</strong></div><div><span style={{color:"#555"}}>Total: </span><strong>{fmt(m*n)}</strong></div>{r>0&&<div style={{gridColumn:"1/-1",color:"#f87171"}}>Interés total: {fmt(m*n-t)}</div>}</div>; })()}
              <button className="btn bp" onClick={addInst}>+ Registrar</button>
            </div>}
          </div>
          <div className="card">
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><div style={{fontFamily:"'Playfair Display',serif",fontSize:14,fontWeight:700}}>Movimientos</div><div style={{fontSize:11,color:"#444"}}>{mTxs.length}</div></div>
            ...
            {mTxs.slice(0,30).map(t=>{
              const cat=CATS.find(c=>c.id===t.category)||CATS[7]; const acc=ACCS.find(a=>a.id===t.account)||ACCS[0]; const isInc=t.type==="income";
        ...
        {view==="cuotas"&&(<>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
            <div className="card" style={{textAlign:"center",padding:14}}><div className="label">Activas</div><div className="stat" style={{fontSize:24,marginTop:4}}>{activeInsts.length}</div></div>
            <div className="card" style={{textAlign:"center",padding:14}}><div className="label">Cargo mes</div><div className="stat" style={{fontSize:20,marginTop:4,color:"#6b6bff"}}>{fmt(instCurrent)}</div></div>
          </div>
          {totalInt>0&&<div className="ay" style={{marginBottom:10}}>💡 Pagarás <strong>{fmt(totalInt)}</strong> en intereses en total con tus cuotas actuales.</div>}
          <div className="card" style={{marginBottom:12}}>
            ...
            {insts.map(inst=>{
              const el=Math.max(0,mDiff(inst.start_month,getCM())); const rem=Math.max(0,inst.months-el); const r=inst.interest_rate/100;
              const monthly=r>0?inst.total_amount*r*Math.pow(1+r,inst.months)/(Math.pow(1+r,inst.months)-1):inst.total_amount/inst.months;
              const interest=monthly*inst.months-inst.total_amount; const pct=Math.min((el/inst.months)*100,100); const active=rem>0;
              const cat=CATS.find(c=>c.id===inst.category)||CATS[7];
              return <div key={inst.id} style={{display:"flex",gap:10,alignItems:"center",padding:"10px 0",borderBottom:"1px solid #181824"}}>
          ...
          <div className="card">
            ...
            {[0,1,2,3,4,5].map(n=>{ const mo=addM(getCM(),n); const ch=instChargeForMonth(mo); const maxC=Math.max(...[0,1,2,3,4,5].map(x=>instChargeForMonth(addM(getCM(),x))),1);
              return <div key={mo} style={{marginBottom:8}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:12,color:n===0?"#6b6bff":"#aaa"}}>{getML(mo)}{n===0?" ←":""}</span><span style={{fontSize:12,fontWeight:600,color:n===0?"#6b6bff":"#e8e4dc"}}>{fmt(ch)}</span></div><div className="bar" style={{height:5}}><div style={{height:"100%",width:`${(ch/maxC)*100}%`,background:n===0?"#6b6bff":"#2a2a50",borderRadius:99,transition:"width 1s"}}/></div></div>;
            })}
          </div>
        </>)}
        ...
        {view==="asesor"&&(<>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
            {["¿Cuánto puedo gastar esta semana?","¿Me conviene pagar de contado o a cuotas?","¿Cómo llego más rápido a mi meta de ahorro?","¿En qué estoy gastando de más?"].map(q2=>(
          ...
          {advice&&<div className="card"><div style={{fontSize:10,color:"#6b6bff",fontWeight:600,textTransform:"uppercase",letterSpacing:.8,marginBottom:10}}>Análisis personalizado</div><div style={{fontSize:14,lineHeight:1.7,color:"#ccc",whiteSpace:"pre-wrap"}}>{advice}</div><button className="btn bg bsm" style={{marginTop:12,width:"100%"}} onClick={()=>{setAdvice("");setAdviceQ("");}}>Nueva consulta</button></div>}
        </>)}
        ...
        {view==="config"&&(<>
          ...
          <div className="card" style={{marginBottom:10}}>
            ...
            {[["💳 Cupo tarjeta","credit_limit"],["🏦 Saldo inicial ahorros","savings_balance"],["💰 Ingreso mensual (nómina)","monthly_income"],["🎯 Meta de ahorro","savings_goal"]].map(([l,k])=>(
              <div key={k} style={{marginBottom:10}}><label style={{fontSize:12,color:"#666",display:"block",marginBottom:3}}>{l}</label><input className="inp" value={settingsForm[k]||""} onChange={e=>setSettingsForm(f=>({...f,[k]:parseInt(e.target.value.replace(/\D/g,""))||0}))} placeholder="$0" /></div>
            ))}
            <div style={{marginBottom:10}}><label style={{fontSize:12,color:"#666",display:"block",marginBottom:3}}>Nombre de la meta</label><input className="inp" value={settingsForm.savings_goal_name||""} onChange={e=>setSettingsForm(f=>({...f,savings_goal_name:e.target.value}))} /></div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
            ...
            <div style={{marginBottom:10}}><label style={{fontSize:12,color:"#666",display:"block",marginBottom:3}}>📧 Palabras clave correos</label><input className="inp" value={settingsForm.email_filter||""} onChange={e=>setSettingsForm(f=>({...f,email_filter:e.target.value}))} /></div>
          </div>
          <div className="card" style={{marginBottom:10}}>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:14,fontWeight:700,marginBottom:12}}>Límites por categoría</div>
            {CATS.map(cat=>(
              <div key={cat.id} style={{marginBottom:8}}><label style={{fontSize:12,color:"#666",display:"block",marginBottom:3}}>{cat.label}</label><input className="inp" placeholder={(S.budgets||{})[cat.id]?fmt((S.budgets||{})[cat.id]):"Sin límite"} value={budgetIn[cat.id]||""} onChange={e=>setBudgetIn(b=>({...b,[cat.id]:e.target.value}))} /></div>
            ))}
          </div>
